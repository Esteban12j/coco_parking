...
pub fn custom_tariffs_list(
    ...
    let (sql, param): (String, Option<String>) = match search.as_deref().map(str::trim) {
        Some(s) if !s.is_empty() => (
            "SELECT id, vehicle_type, plate_or_ref, description, amount, created_at FROM custom_tariffs WHERE plate_or_ref LIKE ?1 OR description LIKE ?1 OR vehicle_type LIKE ?1 ORDER BY vehicle_type ASC, plate_or_ref ASC LIMIT 100".to_string(),
            Some(format!("%{}%", s)),
        ),
        _ => (
            "SELECT id, vehicle_type, plate_or_ref, description, amount, created_at FROM custom_tariffs ORDER BY vehicle_type ASC, COALESCE(plate_or_ref, '') ASC LIMIT 100".to_string(),
            None,
        ),
    };
...
pub fn custom_tariffs_create(
    ...
    conn.execute(
        "INSERT INTO custom_tariffs (id, vehicle_type, plate_or_ref, description, amount, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
        params![id, vehicle_type, plate_key, description, amount, created_at],
    )
    .map_err(|e| e.to_string())?;
...
pub fn custom_tariffs_update(
    ...
    let existing: Option<(String, String, Option<String>, f64)> = conn
        .query_row(
            "SELECT vehicle_type, plate_or_ref, description, amount FROM custom_tariffs WHERE id = ?1",
            params![&id],
    ...
    match existing {
        Some((vt, p, d, a)) => {
            ...
            conn.execute(
                "UPDATE custom_tariffs SET vehicle_type = ?1, plate_or_ref = ?2, description = ?3, amount = ?4 WHERE id = ?5",
                params![new_vehicle_type, new_plate_key, new_description, new_amount, &id],
            )
            .map_err(|e| e.to_string())?;
        }
    ...
    conn.query_row(
        "SELECT id, vehicle_type, plate_or_ref, description, amount, created_at FROM custom_tariffs WHERE id = ?1",
        params![&id],
