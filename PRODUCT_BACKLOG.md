...
| **Frontend** | Rutas: login (Tauri), vehicles, till, metrics, roles, backup, drive, dev-console; useParkingStore, useSession (auth); AuthGate (redirige a /login si no hay sesión en Tauri); LoginPage; RolesPage (dashboard: listado usuarios CRUD, asignación de rol, edición de permisos por rol); VehiculosPage, CajaPage, MetricasPage, CheckoutPanel; i18n es/en. |
