...
**17.2 y 17.3 completadas.** Opción (A) aplicada: todas las llamadas a Tauri pasan por `api/`. Se crearon `api/caja.ts`, `api/vehiculos.ts` y `api/dev.ts`; se corrigió `api/backup.ts` (createBackup con `path`). Features y hooks migrados a usar solo módulos de `api/` (auth, backup, caja, customTariffs, dev, metricas, reportes, roles, vehiculos). `api/index.ts` exporta todos los módulos. No queda uso directo de `invokeTauri` fuera de `api/` ni de `lib/tauriInvoke.ts`. Actualizar tablero/sprint.

**17.4 completada.** Documento creado: `app/src-tauri/COMMAND_TABLE_MAP.md` — mapa comando Tauri → tablas SQL (solo lectura / escritura) para DBA y auditoría. Incluye índice de tablas, tabla Command | Read | Write | Note para todos los comandos registrados en `lib.rs`, y runners de reportes. Instrucciones para mantener el documento al añadir comandos o tablas. **Scrum Master:** marcar 17.4 como Hecho en el tablero/sprint; Épica 17 queda cerrada (17.1–17.4 Hecho).
