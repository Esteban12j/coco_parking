...
| 4.5 | **Dev: password reset** ✅ *Done* | Developer-only flow (e.g. from dev console or hidden route): allow resetting another user's password. Protect by developer login only. **Implemented:** Backend command `dev_reset_user_password(user_id, new_password)` in `dev.rs`; gated by `require_dev_console` (dev mode + `dev:console:access`). Dev Console page has a "Reset user password" section: user ID + new password (min 4 chars); success/error toasts. i18n en/es. | 4.3 |
| 4.6 | **Security doc** ✅ *Done* | Short note in security/docs: dev account purpose, that it is hidden, and that permissions are restricted to recovery/administration. **Implemented:** `security/docs/DEVELOPER_ACCOUNT.md` documents purpose (recovery/administration), visibility (hidden from UI and list APIs), and restricted permissions (backup + dev console). | — |
...
**Scrum Master — update (ticket 4.6):** Ticket **4.6 (Security doc)** is **done**. A short security note was added at `security/docs/DEVELOPER_ACCOUNT.md` describing the developer account: its purpose (recovery and administration only), that it is hidden from the roles/users UI and list APIs, and that its permissions are restricted to backup (list/create/restore/config) and dev console access. You can mark 4.6 completed and consider Epic 4 (First install: admin password + dev account) closed.

**Scrum Master — update (ticket 4.5):** Ticket **4.5 (Dev: password reset)** is **done**. Developer-only password reset is available from the Dev Console: backend command `dev_reset_user_password` (protected by dev mode + `dev:console:access`), and a dedicated "Reset user password" card on the Dev Console page (user ID + new password, min 4 characters). Only users who have logged in as developer (or have dev console access) can use it. You can mark 4.5 completed.
