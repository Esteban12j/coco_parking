...
**Siguiente en desarrollo:** **10.3** (mapa de calor día de semana vs tipo de vehículo). Luego 10.5 (filtros y tabla en reportes), 10.4 (rango fechas).  
...
**Nota para Scrum Master:** Historias **10.1**, **10.2** y **10.3** están **Hecho**. **10.3** (Mapa de calor día de semana vs tipo de vehículo) entregado: backend `metricas_get_heatmap_day_vehicle` (date_from, date_to, period opcional); frontend widget en fila completa con heatmap legible (días vs tipo de vehículo, tipos dinámicos desde BD), leyenda bajo/medio/alto, tooltips, selector de fechas y filtro de periodo del día (mañana, medio día, tarde, noche, sin filtro). **Orden de desarrollo Épica 10:** 10.5 (filtros + tabla reportes) → 10.4 (rango fechas). **Actualizar tablero/sprint** moviendo 10.3 a Hecho.
